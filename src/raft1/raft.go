...
func (rf *Raft) persist() {
	...
	rf.persister.Save(raftstate, rf.Snap.Snapshot)
}
	...
	for !rf.killed() {
		...
		if PrevLogIndex < rf.Log.Index0 {
			...
			if result.ok {
				...
				rf.nextIndex[peer] = rf.Snap.LastIncludedIndex + 1
				rf.matchIndex[peer] = rf.Snap.LastIncludedIndex
			} else {
				// RPC 失败，等待一段时间后重试
